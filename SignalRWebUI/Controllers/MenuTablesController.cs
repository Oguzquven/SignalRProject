...
namespace SignalRWebUI.Controllers
{
	public class MenuTablesController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			var responseMessage = await client.GetAsync("https://localhost:7010/api/MenuTables/");
		...
		public async Task<IActionResult> CreateMenuTable(CreateMenuTableDto createMenuTableDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7010/api/MenuTables", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"https://localhost:7010/api/MenuTables/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync($"https://localhost:7010/api/MenuTables/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
		...
		public async Task<IActionResult> UpdateMenuTable(UpdateMenuTableDto updateMenuTableDto)
		{
			...
			StringContent stringContent = new StringContent(jsondata, Encoding.UTF8, "application/json");
			var responseMessage = await client.PutAsync("https://localhost:7010/api/MenuTables/", stringContent);
			if ((responseMessage.IsSuccessStatusCode))
			{
				return RedirectToAction("Index");
			}
